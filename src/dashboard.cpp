...
void Dashboard::drawBitmapFromSD(File file, int16_t x, int16_t y, bool with_color) 
{
...
void Dashboard::drawBitmapFile(File file) {    
}

uint16_t Dashboard::read16(File& f)
{
...
uint32_t Dashboard::read32(File& f)
{
  // BMP data is stored little-endian, same as Arduino.
  uint32_t result;
  ((uint8_t *)&result)[0] = f.read(); // LSB
